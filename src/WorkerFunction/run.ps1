...
<#
.SYNOPSIS
    Queue-triggered worker function - Executes the SQL Edition Optimisation workflow.
    Triggered by HttpStartFunction or ScheduledTrigger via the 'scan-requests' queue.
    Runs asynchronously with no HTTP timeout constraints (up to 30 min function timeout).
#>

Write-Host "WorkerFunction: Starting SQL Edition Optimisation workflow..."
Write-Host "WorkerFunction: Queue message received at $(Get-Date -Format 'o')"

try {
    # Parse input from queue message
    $payload = if ($QueueItem -and $QueueItem -ne '{}') {
        try { $QueueItem | ConvertFrom-Json } catch { @{} }
    } else { @{} }
    ...
    Write-Host "Step 1 complete: Found $($allMachines.Count) Arc machines with SQL Server"

    if ($allMachines.Count -eq 0) {
        Write-Host "No machines found - nothing to do."
        return
    }
    ...
    foreach ($machine in $allMachines) {
        ...
        while ($attempt -lt $maxRetries -and -not $success) {
            ...
            try {
                ...
                $success = $true
                $results.Add(@{
                    MachineName    = $machineName; ResourceGroup = $resourceGroup
                    ...
                    Timestamp      = (Get-Date -Format 'o'); Attempts = $attempt
                })
            }
            catch {
        ...
        if (-not $success) {
            $results.Add(@{
                MachineName = $machineName; ResourceGroup = $resourceGroup
                SubscriptionId = $subscriptionId; Location = $location
                ExecutionState = 'Failed'; Output = $null
    ...
    foreach ($result in $results) {
        if ($result.ExecutionState -ne 'Failed' -and $result.Output) {
            try {
                ...
                $logEntries.Add(@{
                    TimeGenerated = $result.Timestamp; MachineName = $result.MachineName
                    InstanceName = 'ParseError'; Edition = 'Unknown'
            ...
            $logEntries.Add(@{
                TimeGenerated = $result.Timestamp; MachineName = $result.MachineName
                InstanceName = 'ExecutionFailed'; Edition = 'Unknown'
    ...
    Write-Host "Workflow complete: $successful successful, $failed failed, $($ingestionResult.Status) ingestion"
}
catch {
    Write-Host "WorkerFunction ERROR: $($_.Exception.Message)"
    Write-Host "WorkerFunction STACK: $($_.ScriptStackTrace)"
    throw
}
